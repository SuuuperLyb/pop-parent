...
import com.offway.hqs.expection.SmsException;
...
import com.offway.hqs.util.RUtil;
...
public class TSmsLogServiceImpl extends ServiceImpl<TSmsLogMapper,TSmsLogDto> implements ITSmsLogService {
    ...
    public R sendRegisterCode(String phone) {
        ...
        try {
            ...
            logDao.insert(log);
            // 将产生的6位字符串添加到redis中去,key为 (hungry:sms:rcode:+手机号)  值为code字符串   时间为5分钟
            jedisCore.set(RedisKeyConfig.SMS_RCODE + phone, String.valueOf(code), RedisKeyConfig.SMS_RCODE_TIME);
    ...
    public R checkRCode(UserRegisterCodeDto codeDto) {
        ...
        // 将 key为hungry:sms:rcode:+用户的手机号 值为 当时生成的验证码 保存到了redis中
        if (jedisCore.checkKey(RedisKeyConfig.SMS_RCODE + codeDto.getPhone())) {
            // 此时指的是在 已经点击过了获取验证码，但是验证码还没有过期的时间段内，此时自己的 hungry:sms:rcode:+用户的手机号 作为key
            ...
            if (codeDto.getCode() == Integer.parseInt(jedisCore.get(RedisKeyConfig.SMS_RCODE + codeDto.getPhone()))) {
                ...
                return RUtil.error("亲，验证码不正确！");
            }
        } else {
            return RUtil.error("验证码已失效");
        }
    }
}
